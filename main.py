          ...
          <div id="tool-settings" class="tool-view">
              ...
              <div class="tool-container" style="margin-bottom:20px;">
                  <div class="input-group">
                      ...
                      <input type="text" id="masterPathInput" value="/storage/emulated/0/Mehar Pro Downloader">
      ...
      <script>
        ...
        window.onload = () => {
            ...
            if(savedBase) {
                ...
                document.getElementById('folderPath').value = savedBase + "/TikTok";
                document.getElementById('uniFolderPath').value = savedBase + "/Universal";
                document.getElementById('ytFolderPath').value = savedBase + "/YT_Shorts";
                document.getElementById('convFolder').value = savedBase + "/TikTok";
            }
        }
        ...
        async function convertImages() {
            ...
            try {
                ...
                statusBox.innerHTML = data.status === "error" ? `❌ Error: ${data.message}` : `✅ ${data.message}`;
        ...
        async function download(mode) {
          ...
          try {
            const res = await fetch('/download', {
              ...
              body: JSON.stringify({ selectedUrls: selected, folderPath: document.getElementById('folderPath').value.trim(), quality: document.getElementById('quality').value, saveMode: document.getElementById('saveMode').value })
            });
            const data = await res.json();
            if(data.status === "error") document.getElementById('status1').innerHTML = `❌ Error: ${data.error}`;
            else { document.getElementById('status1').innerHTML = `✅ Saved in: ${data.location}`; showToast("TikTok Batch Done"); addHistory(`TikTok Batch: ${selected.length} items`); }
        ...
        async function startUniversal() {
            ...
            try {
                const res = await fetch('/universal-download', {
                    method: 'POST', headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ url: url, folderPath: document.getElementById('uniFolderPath').value.trim(), formatType: document.getElementById('uniType').value, quality: document.getElementById('uniQuality').value })
                });
                const data = await res.json();
                if(data.status === "error") document.getElementById('status2').innerHTML = `❌ Error: ${data.error}`;
                else { document.getElementById('status2').innerHTML = `✅ Saved in: ${data.location}`; showToast("Media Downloaded"); addHistory("Universal Extract"); }
        ...
        async function startYtShorts() {
            ...
            try {
                const res = await fetch('/process-yt-document', {
                    method: 'POST', headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ document_text: docText, save_path: document.getElementById('ytFolderPath').value.trim(), mode: document.getElementById('ytSaveMode').value, quality: document.getElementById('ytQuality').value })
                });
                const data = await res.json();
                if(data.status === "error") document.getElementById('status3').innerHTML = `❌ Error: ${data.message}`;
                else { document.getElementById('status3').innerHTML = `✅ Sliced to: ${data.location}`; showToast("Shorts Extracted"); addHistory("YT Shorts Maker"); }
        ...
        async function convertMp3BulkFolder() {
            const path = document.getElementById('convFolder').value.trim(); if(!path) return;
            document.getElementById('status4').innerText = "Processing Directory...";
            try {
                const res = await fetch('/convert-mp3-folder', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ folderPath: path }) });
                const data = await res.json();
                document.getElementById('status4').innerHTML = data.status === "error" ? `❌ Error: ${data.message}` : `✅ ${data.message}`;
                if(data.status === "done") { showToast("Folder MP3 Ready"); addHistory("MP3 Bulk Folder"); }
            } catch(e) { document.getElementById('status4').innerHTML = `❌ Error`; }
        }
      </script>
    </body>
    </html>
...
def get_safe_default_path(subfolder): return BASE_DIR / subfolder
...
def download(req: DownloadRequest):
    ...
    try:
        ...
        return JSONResponse({"status": "done", "location": final_location})
...
def universal_download(req: UniversalRequest):
    ...
    try:
        ...
        progress_state["is_active"] = False
        return JSONResponse({"status": "done", "location": str(folder)})
    except Exception as e:
        progress_state["is_active"] = False
        return JSONResponse({"status": "error", "error": str(e)}, status_code=500)
...
def process_yt_document(req: YTDocRequest):
    ...
    folder_path = Path(req.save_path.strip()).resolve() if req.save_path.strip() else get_safe_default_path("YT_Shorts")
    ...
    progress_state["is_active"] = False
    return JSONResponse({"status": "done", "location": final_loc})
...
async def convert_mp3_upload(files: List[UploadFile] = File(...)):
    ...
    shutil.rmtree(temp_dir, ignore_errors=True)
    return JSONResponse({"status": "done", "message": f"Converted {converted} file(s).<br>Saved in: {mp3_dir}"})
...
async def convert_image_format(files: List[UploadFile] = File(...), target_format: str = Form(...)):
    ...
    return JSONResponse({"status": "done", "message": f"{converted} Images saved in:<br>{out_dir}"})
...
async def images_to_pdf(files: List[UploadFile] = File(...)):
    ...
    try:
        ...
        scan_media(str(out_file))
        return JSONResponse({"status": "done", "message": f"PDF created successfully!<br>Saved in: {out_file}"})
    except Exception as e:
        return JSONResponse({"status": "error", "message": str(e)})
